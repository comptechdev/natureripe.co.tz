...
<div class="grid grid-cols-1 md:grid-cols-3">
    @foreach ($sauces as $product)
        <x-website.about.recepies href="{{ route('website.sauce', ['product' => $product]) }}" background="{{ Storage::url($product->background_texture) }}" bottle="{{ Storage::url($product->product_image_transparent) }}" title="{{ $product->name }}" titlestyle="text-{{ $product->color }}-600 text-2xl font-extrabold" description="{{ $product->description }}" /> 
    @endforeach
</div>
...
@foreach ($groups as $group)
    <div class="max-w-5xl mx-auto">
        <div class="container mx-auto py-5">
            <h1 class="text-3xl font-extrabold pt-16 pb-10">{{ $group->name }}</h1>
